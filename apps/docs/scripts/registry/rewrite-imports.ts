import type { SymbolEntry } from "./symbol-map";
...
// Relative import of the shared cn/utils helper, e.g. "../../lib/utils", "../lib/utils".
// Requires at least one "../" segment — a same-level "./lib/utils" would NOT match this.
...
export function rewriteImports(
  ...
  let code = source.replace(BARREL, (_full, clause: string) => {
    ...
    for (const spec of parseSpecifiers(clause)) {
      const entry = symbolMap.get(spec.name);
      if (!entry) throw new Error(`Unknown @manpowerhub/ui export: ${spec.name}`);
      registryDeps.add(entry.item);
      const aliasPath = entry.file === "utils" ? "@/lib/utils" : `@/components/ui/${entry.file}`;
